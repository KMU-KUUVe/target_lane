...
#include "target_lane/TargetLane.h"
...
cv::Mat TargetLane::deNoise(cv::Mat inputImage) {
	cv::Mat output;
...
void TargetLane::filter_colors(Mat _img_bgr, Mat &img_filtered)
{
...
1 : rectangle ROI
 */
cv::Mat TargetLane::mask(cv::Mat frame) {
	cv::Mat output;
